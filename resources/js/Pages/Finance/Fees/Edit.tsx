...
export default function Edit({ auth, fee, academicYears }: PageProps<{ fee: FeeStructure, academicYears: AcademicYear[] }>) {
    ...
    const submit = (e: React.FormEvent) => {
        e.preventDefault();
        put(route('finance.fees.update', fee.id));
    };

    const handleDelete = () => {
        if (confirm('Are you sure you want to delete this fee structure? This cannot be undone.')) {
            router.delete(route('finance.fees.destroy', fee.id));
        }
    };
        ...
        <FinanceLayout
            header={
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="font-extrabold text-xl text-slate-800 leading-tight">Edit Fee Structure</h2>
                        <p className="text-xs text-slate-400 font-semibold">Modify fee configurations, amounts, or restrictions</p>
                    </div>
                </div>
            }
        >
                        ...
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="flex items-center justify-between border-t border-slate-100 pt-6">
                                <button
                                    ...
                                    className="inline-flex items-center justify-center px-4 py-2.5 text-xs font-black text-red-600 hover:bg-red-50 rounded-xl transition-all uppercase tracking-wider"
                                >
                                    Delete Structure
                                </button>
